...
CREATE POLICY "storage_public_read_20250120"
ON storage.objects FOR SELECT
TO public
USING (bucket_id = 'images');

CREATE POLICY "storage_public_write_20250120"
...
BEGIN
  ...
  SELECT 'images', 'generated/.keep', auth.uid(), now(), now(), '{"mimetype": "text/plain"}'::jsonb
  WHERE NOT EXISTS (
    SELECT 1 FROM storage.objects 
    WHERE bucket_id = 'images' AND name = 'generated/.keep'
  );
END $$;
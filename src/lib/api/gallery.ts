...
export async function getGalleryTypes(): Promise<string[]> {
  ...
  try {
    ...
    if (error) throw error;
    return (data || []).map((gallery) => gallery.name);
  } catch (error) {
...
export async function updateGallery(oldName: string, newName: string): Promise<void> {
  const supabase = getSupabaseClient();
  if (!supabase) throw new Error('Supabase client not initialized');
...
export async function deleteGallery(name: string): Promise<void> {
  const supabase = getSupabaseClient();
  if (!supabase) throw new Error('Supabase client not initialized');

  try {
    // First, get the gallery type ID
    const { data: galleryType, error: galleryTypeError } = await supabase
      .from(TABLES.GALLERY_TYPE)
      .select('id')
      .eq('name', name)
      .single();

    if (galleryTypeError) throw galleryTypeError;

    // Update any images in this gallery to have no gallery type
    await supabase
      .from(TABLES.GALLERY)
      .update({ gallery_type: null })
      .eq('gallery_type', galleryType.id);
